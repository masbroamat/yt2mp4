...
export default function Home() { 
  ...
  useEffect(() => {
    ...
    if (!lastClickTime || now - parseInt(lastClickTime) > AFFILIATE_RESET_TIME) {
      resetAffiliateCycle();
    } else {
      const storedCount = parseInt(localStorage.getItem("downloadCount") || "0", 10);
  ...
  const handleDownload = async (formatId) => {
    ...
    const selectedOption = qualityOptions.find(opt => opt.format_id === formatId);
        ...
        <div className="w-full max-w-3xl mx-auto px-8">
          ...
          <div className="fixed top-0 left-0 w-full bg-white text-black py-2 z-50 opacity-[70%]">
            <p className="text-center text-xs font-medium">
              Announcement: Scheduled maintenance daily from 6:00 AM to 7:00 AM. Downloads may be temporarily unavailable.
            </p>
          </div>
          ...
          <div className="space-y-6">
            ...
            {noOptionsAvailable && (
              <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400">
                This video currently does not have a download link. Please try again tomorrow.
              </div>
            )}
            
            {showQualityOptions && qualityOptions.length > 0 && (
              <div>
                <h3 className="text-lg font-medium text-white mb-4">Available Quality Options:</h3>
            ...
            {downloadLink && isDownloadReady && (
              <div className="p-6 bg-white/5 border border-white/10 rounded-lg">
                <p className="text-white mb-4">
                  Your video is ready to download {selectedQualityLabel ? `(${selectedQualityLabel})` : ''}:
                </p>
                <button 
